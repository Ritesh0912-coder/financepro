...
const GooeyNav: React.FC<GooeyNavProps> = ({
  ...
  const filteredItems = items.filter(item => {
    if (item.label === 'Admin' && (!session || (session.user as any).role !== 'admin')) return false;
    if (item.label === 'Login' && session) return false; // Hide Login when logged in
    return true;
  });

  // Add Profile link if logged in
  if (session && !filteredItems.find(i => i.label === 'Profile')) {
    filteredItems.push({ label: 'Profile', href: '/user/profile' });
  }
