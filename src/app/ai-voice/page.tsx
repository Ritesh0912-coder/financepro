...
import {
    Mic, X, Plus, Search, Send, Sparkles, User, Globe, ArrowUp, Volume2, MessageSquare, Menu, ChevronLeft
} from 'lucide-react';
...
export default function AiVoicePage() {
    ...
    const handleUserQuery = async (text: string, mode: 'text' | 'voice') => {
        ...
        try {
            ...
            if (data.reply) {
                ...
                // If it was a new session, backend returns the new sessionId
                if (!currentSessionId && data.sessionId) {
                    setCurrentSessionId(data.sessionId);
                    // Refresh session list to show new chat in sidebar
                    const sessionRes = await fetch('/api/ai-chat?mode=list');
    ...
    return (
        <div className="flex h-screen bg-[#060606] text-white overflow-hidden font-sans selection:bg-orange-500/30">

            {/* --- Sidebar --- */}
            <motion.div
                initial={false}
                animate={{ width: isSidebarOpen ? 280 : 0, opacity: isSidebarOpen ? 1 : 0 }}
                className="bg-[#0a0a0a] border-r border-white/5 flex-shrink-0 relative z-20 overflow-hidden"
            >
                <div className="p-4 flex flex-col h-full w-[280px]"> {/* Fixed width container to prevent squash */}
                    <Button
                        onClick={startNewChat}
                        className="w-full bg-white/5 hover:bg-white/10 text-white border border-white/10 justify-start gap-2 h-10 mb-6"
                    >
                    ...
                    <div className="flex-1 overflow-y-auto no-scrollbar space-y-1">
                        ...
                        {sessions.map((sess) => (
                            <button
                                ...
                                className={cn(
                                    "w-full text-left px-3 py-2.5 rounded-lg text-sm transition-all truncate flex items-center gap-2",
            ...
            <div className="flex-1 relative flex flex-col h-full overflow-hidden">
                ...
                <div className="flex-1 flex flex-col items-center justify-center p-4 relative">
                    ...
                    <AnimatePresence mode="wait">
                        {!hasInteraction ? (
                            ...
                            <motion.div
                                ...
                                initial={{ opacity: 0 }}
                                animate={{ opacity: 1 }}
                                className="w-full max-w-3xl flex flex-col gap-6 py-6 pb-32 h-full overflow-y-auto no-scrollbar scroll-smooth"
                            >
                                <div className="flex justify-center mb-4 sticky top-0 z-10 py-4 bg-gradient-to-b from-[#060606] via-[#060606]/95 to-transparent">
                                    <div className="scale-50 opacity-50 transform transition-transform hover:scale-55 cursor-pointer" onClick={() => { }}>
                                        <GlowingOrb state={state} />
                                    </div>
                                </div>
                    ...
                    <div className={cn(
                        "fixed bottom-8 w-full max-w-2xl px-4 z-50 transition-all duration-300",
                        isSidebarOpen ? "left-[calc(50%+140px)] -translate-x-1/2" : "left-1/2 -translate-x-1/2"
                    )}>
                        <div className="bg-[#121212]/90 border border-white/10 rounded-full p-1.5 px-5 backdrop-blur-2xl flex items-center gap-3 shadow-[0_15px_35px_rgba(0,0,0,0.6)] hover:border-orange-500/20 transition-all">
                            <button
                                onClick={startNewChat}
                                className="h-9 w-9 flex items-center justify-center text-slate-500 hover:text-white transition-colors"
                                title="New Chat"
                            >
